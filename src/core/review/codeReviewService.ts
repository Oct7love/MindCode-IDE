...
const SECURITY_PATTERNS: {
...
}[] = [
  {
    pattern: /eval\s*\(/g,
    title: "eval() 使用",
    message: "避免使用 eval()，可能导致代码注入攻击",
    severity: "error",
  },
  {
    pattern: /innerHTML\s*=/g,
    title: "innerHTML 赋值",
    message: "直接赋值 innerHTML 可能导致 XSS 攻击，建议使用 textContent 或 DOMPurify",
    severity: "warning",
  },
  {
    pattern: /dangerouslySetInnerHTML/g,
    title: "dangerouslySetInnerHTML",
    message: "React 中使用 dangerouslySetInnerHTML 需确保内容已消毒",
    severity: "warning",
  },
  {
    pattern: /password\s*[:=]\s*['"][^'"]+['"]/gi,
    title: "硬编码密码",
    message: "检测到硬编码密码，应使用环境变量或密钥管理服务",
    severity: "error",
  },
  {
    pattern: /api[_-]?key\s*[:=]\s*['"][^'"]+['"]/gi,
    title: "硬编码 API Key",
    message: "检测到硬编码 API Key，应使用环境变量",
    severity: "error",
  },
  {
  ...
  },
  {
    pattern: /exec\s*\(\s*['"`].*\$\{/g,
    title: "命令注入风险",
    message: "字符串插值用于 exec() 可能导致命令注入",
    severity: "error",
  },
  {
    pattern: /SELECT\s+\*\s+FROM.*\+\s*\w+/gi,
    title: "SQL 注入风险",
    message: "字符串拼接 SQL 查询可能导致 SQL 注入，使用参数化查询",
    severity: "error",
  },
  {
    pattern: /new\s+Function\s*\(/g,
    title: "Function 构造器",
    message: "使用 Function 构造器类似 eval()，存在安全风险",
    severity: "warning",
  },
  {
    pattern: /document\.write/g,
    title: "document.write",
    message: "document.write 可能导致 XSS 和性能问题",
    severity: "warning",
  },
];
...
const PERFORMANCE_PATTERNS: {
...
}[] = [
  {
    ...
    title: "循环中重复计算 length",
    message: "将 .length 缓存到变量中以提高性能",
    severity: "info",
  },
  {
    pattern: /JSON\.parse\(JSON\.stringify/g,
    title: "深拷贝使用 JSON",
    message: "使用 structuredClone() 或 lodash.cloneDeep 替代",
    severity: "info",
  },
  {
    pattern: /\.\s*forEach\s*\([^)]*await/g,
    title: "forEach 中使用 await",
    message: "forEach 不会等待 async 回调，使用 for...of 或 Promise.all",
    severity: "warning",
  },
  {
    pattern: /new\s+RegExp\s*\([^)]+\)/g,
    title: "循环内创建 RegExp",
    message: "如果模式不变，在循环外创建 RegExp 对象",
    severity: "info",
  },
  {
    pattern: /console\.(log|debug|info)/g,
    title: "生产环境 console",
    message: "生产环境应移除或禁用 console 输出",
    severity: "info",
  },
  {
    pattern: /setTimeout\s*\(\s*['"][^'"]*['"]/g,
    title: "setTimeout 字符串参数",
    message: "setTimeout 使用字符串参数类似 eval()，使用函数代替",
    severity: "warning",
  },
];
...
const STYLE_PATTERNS: {
...
}[] = [
  {
    pattern: /var\s+\w+/g,
    title: "使用 var",
    message: "推荐使用 const 或 let 替代 var",
    severity: "info",
  },
  {
    pattern: /==(?!=)/g,
    title: "使用 == 比较",
    message: "推荐使用 === 进行严格相等比较",
    severity: "info",
  },
  {
    pattern: /!=(?!=)/g,
    title: "使用 != 比较",
    message: "推荐使用 !== 进行严格不等比较",
    severity: "info",
  },
  {
    pattern: /function\s*\([^)]*\)\s*\{[^}]{200,}/g,
    title: "函数过长",
    message: "函数超过 200 字符，考虑拆分为更小的函数",
    severity: "info",
  },
  {
    pattern: /TODO|FIXME|HACK|XXX/g,
    title: "待办注释",
    message: "检测到 TODO/FIXME 注释，记得处理",
    severity: "info",
  },
];
...
class CodeReviewService {
  ...
  autoFix(
    content: string,
    issue: CodeIssue,
  ): { fixed: string; applied: boolean; description: string } {
    const lines = content.split("\n");
    const lineIdx = issue.line - 1;
    if (lineIdx < 0 || lineIdx >= lines.length)
      return { fixed: content, applied: false, description: "行号超出范围" };
    ...
    if (!rule.pattern.test(line))
      return { fixed: content, applied: false, description: "未匹配到问题代码" };
  ...
  autoFixAll(
    content: string,
    issues: CodeIssue[],
  ): { fixed: string; fixedCount: number; details: string[] } {
    let result = content;
    ...
    for (const issue of sorted) {
      ...
      if (applied) {
        result = fixed;
        fixedCount++;
        details.push(`L${issue.line}: ${description}`);
      }
    }
...
${content
  .split("\n")
  .slice(Math.max(0, issue.line - 5), issue.line + 5)
  .join("\n")}
\`\`\`
...
REASON: <解释>`;
    try {
      const res = await window.mindcode.ai.chat("claude-sonnet-4-5-20250929", [
