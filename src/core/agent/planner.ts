...
5. 先分析再执行`;
   ...
   * 生成简单计划（AI 失败时的后备）
   */
  private generateSimplePlan(userRequest: string): any {
    // 分析请求类型
   ...
   * 构建计划对象
   */
  private buildPlan(planData: any, userRequest: string): TaskPlan {
    const planId = `plan-${Date.now()}`;
    
    const subtasks: SubTask[] = (planData.subtasks || []).map((st: any, index: number) => ({
      id: `${planId}-task-${index}`,
      parentId: planId,
      name: st.name || `子任务 ${index + 1}`,
      description: st.description || '',
      type: st.type || 'code_modification',
      priority: st.priority || 'medium',
      status: 'pending' as TaskStatus,
      dependsOn: (st.dependsOn || []).map((d: string | number) => `${planId}-task-${d}`),
    }));
    
    return {
      id: planId,
      name: planData.name || '执行计划',
  ...
  private detectDependencies(plan: TaskPlan): void {
    ...
    for (let i = 1; i < plan.subtasks.length; i++) {
      ...
      if (task.type === 'testing' || task.name.includes('验证')) {
        const execTasks = plan.subtasks.filter(t => 
          t.type === 'code_generation' || 
          t.type === 'code_modification' ||
          t.type === 'bug_fix'
        );
