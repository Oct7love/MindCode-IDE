...
import type { RefactorRequest, ExecutionPlan, ImpactAnalysis } from "./types";
...
export async function analyzeImpact(
  ...
  const analysis: ImpactAnalysis = {
    affectedFiles: Array.from(affectedFiles).map((filePath) => {
      const action = plan.steps.flatMap((s) => s.actions).find((a) => a.filePath === filePath);

      return {
        filePath,
        changeType:
          action?.type === "create" ? "create" : action?.type === "delete" ? "delete" : "modify",
        reason: "由重构计划确定",
        riskLevel:
          action?.type === "delete" ? "high" : action?.type === "create" ? "low" : "medium",
      };
    }),
    dependencies,
    riskSummary: {
      ...
      estimatedComplexity:
        affectedFiles.size > 10 ? "complex" : affectedFiles.size > 5 ? "moderate" : "simple",
    },
  };
