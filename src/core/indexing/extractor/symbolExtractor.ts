...
import { TypeScriptParser, ParseResult } from '../parser/typescript';
...
export class SymbolExtractor {
   ...
   * 提取文件中的符号信息
   */
  extract(filePath: string, sourceCode: string): {
    symbols: CodeSymbol[];
    chunks: CodeChunk[];
  ...
  private generateChunks(
    ...
    for (const symbol of symbols) {
      ...
      if (chunkLines > this.config.maxChunkLines) {
        // 分割成多个片段
        for (let start = symbol.startLine; start <= symbol.endLine; start += this.config.maxChunkLines) {
