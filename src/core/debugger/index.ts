...
export interface LaunchConfig {
  name: string;
  type: "node" | "chrome" | "python" | "go" | "rust";
...
type DebugEventHandler = (event: string, data: unknown) => void;

/** 调试器全局 API 接口 */
interface DebugWindow {
  mindcode?: {
    debug?: {
      start?: (config: LaunchConfig) => Promise<{ success: boolean }>;
      stop?: (id: string) => Promise<void>;
      continue?: (id: string) => Promise<void>;
      stepOver?: (id: string) => Promise<void>;
      stepInto?: (id: string) => Promise<void>;
      stepOut?: (id: string) => Promise<void>;
      pause?: (id: string) => Promise<void>;
      setBreakpoints?: (
        file: string,
        bps: Array<{ line: number; condition?: string }>,
      ) => Promise<void>;
      getVariables?: (sessionId: string, frameId?: number) => Promise<Variable[]>;
      evaluate?: (
        sessionId: string,
        expression: string,
        frameId?: number,
...
const win = (typeof window !== "undefined" ? window : null) as (Window & DebugWindow) | null;
...
class DebuggerManager {
  ...
  async stopSession(sessionId?: string): Promise<void> {
    ...
    try {
      if (win?.mindcode?.debug?.stop) await win?.mindcode.debug.stop(id);
    } catch (e) {
      console.error("[Debugger] 停止失败:", e);
    }
  ...
  getSession(sessionId?: string): DebugSession | undefined {
    return this.sessions.get(sessionId || this.activeSessionId || "");
  }
  ...
  async continue(sessionId?: string): Promise<void> {
    ...
    if (!id) return;
    try {
      if (win?.mindcode?.debug?.continue) await win?.mindcode.debug.continue(id);
      this.updateState(id, "running");
    } catch (e) {
      console.error("[Debugger] Continue 失败:", e);
    }
  }
  ...
  async stepOver(sessionId?: string): Promise<void> {
    ...
    if (!id) return;
    try {
      if (win?.mindcode?.debug?.stepOver) await win?.mindcode.debug.stepOver(id);
    } catch (e) {
      console.error("[Debugger] StepOver 失败:", e);
    }
  }
  ...
  async stepInto(sessionId?: string): Promise<void> {
    ...
    if (!id) return;
    try {
      if (win?.mindcode?.debug?.stepInto) await win?.mindcode.debug.stepInto(id);
    } catch (e) {
      console.error("[Debugger] StepInto 失败:", e);
    }
  }
  ...
  async stepOut(sessionId?: string): Promise<void> {
    ...
    if (!id) return;
    try {
      if (win?.mindcode?.debug?.stepOut) await win?.mindcode.debug.stepOut(id);
    } catch (e) {
      console.error("[Debugger] StepOut 失败:", e);
    }
  }
  ...
  async pause(sessionId?: string): Promise<void> {
    ...
    if (!id) return;
    try {
      if (win?.mindcode?.debug?.pause) await win?.mindcode.debug.pause(id);
      this.updateState(id, "paused");
    } catch (e) {
      console.error("[Debugger] Pause 失败:", e);
    }
  }
  ...
  removeBreakpoint(id: string): void {
    for (const [file, bps] of this.breakpoints) {
      ...
      if (idx !== -1) {
        const [removed] = bps.splice(idx, 1);
        this.syncBreakpoints(file);
  ...
  private async syncBreakpoints(file: string): Promise<void> {
    ...
    try {
      await win?.mindcode.debug.setBreakpoints(
        file,
        bps.filter((bp) => bp.enabled).map((bp) => ({ line: bp.line, condition: bp.condition })),
      );
    } catch (e) {
      console.error("[Debugger] 同步断点失败:", e);
    }
  }
  ...
  async getVariables(frameId?: number): Promise<Variable[]> {
    ...
    try {
      if (win?.mindcode?.debug?.getVariables)
        return await win?.mindcode.debug.getVariables(session.id, frameId);
    } catch (e) {
      console.error("[Debugger] 获取变量失败:", e);
    }
    return [];
  }
  ...
  async evaluate(expression: string, frameId?: number): Promise<string> {
    ...
    if (!session) return "";
    try {
      if (win?.mindcode?.debug?.evaluate) {
        const result = await win?.mindcode.debug.evaluate(session.id, expression, frameId);
        return result?.value || "";
      }
    } catch (e) {
      console.error("[Debugger] 求值失败:", e);
    }
    return "";
  }
