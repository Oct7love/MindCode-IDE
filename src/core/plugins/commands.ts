...
export class CommandRegistry {
  ...
  registerCommand(
    id: string,
    handler: CommandHandler,
    options?: { title?: string; category?: string; source?: string },
  ): Disposable {
  ...
  async executeCommand(id: string, ...args: unknown[]): Promise<unknown> {
    ...
    if (!info) {
      console.error(`[CommandRegistry] 命令未找到: ${id}`);
  ...
  /** 获取命令信息 */
  getCommand(id: string): CommandInfo | undefined {
  ...
  /** 按来源筛选命令 */
  listCommandsBySource(source: string): CommandInfo[] {
    return this.listCommands().filter((c) => c.source === source);
  }
...
export function getCommandRegistry(): CommandRegistry {
  if (!_registry) _registry = new CommandRegistry();
  return _registry;
}
