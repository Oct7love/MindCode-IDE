...
export interface CollabUser { id: string; name: string; color: string; cursor?: { file: string; line: number; column: number }; }
export interface CollabSession { id: string; name: string; host: string; users: CollabUser[]; createdAt: number; status: 'active' | 'ended'; }
export interface CollabEdit { userId: string; file: string; range: { startLine: number; startCol: number; endLine: number; endCol: number }; text: string; timestamp: number; }

type CollabEventHandler = (event: string, data: any) => void;
...
class CollabService {
  ...
  /** 创建协作会话 (占位) */
  async createSession(name: string): Promise<{ success: boolean; sessionId?: string; error?: string }> {
    this.session = { id: `session-${Date.now()}`, name, host: this.userId, users: [{ id: this.userId, name: this.userName, color: this.randomColor() }], createdAt: Date.now(), status: 'active' };
  ...
  async joinSession(sessionId: string): Promise<{ success: boolean; error?: string }> {
    ...
    this.session = { id: sessionId, name: 'Joined Session', host: 'remote', users: [user], createdAt: Date.now(), status: 'active' };
  ...
  /** 获取当前会话 */
  getSession(): CollabSession | null { return this.session; }
