...
export class CodesucProvider extends BaseAIProvider { // 特价渠道 - Electron net 模块（纯文本 Agent Loop）
  ...
  models: ModelInfo[] = [
    { id: 'codesuc-opus', name: 'Claude Opus 4.5 [特价]', contextWindow: 200000, inputPrice: 10, outputPrice: 50 },
    { id: 'codesuc-sonnet', name: 'Claude Sonnet 4.5 [特价]', contextWindow: 200000, inputPrice: 2, outputPrice: 10 },
    { id: 'codesuc-haiku', name: 'Claude Haiku 4.5 [特价]', contextWindow: 200000, inputPrice: 0.25, outputPrice: 1.25 },
  ];
  ...
  private capabilitiesCache: { tools: boolean; stream: boolean; probed: boolean } = { tools: false, stream: true, probed: false };

  constructor(config: AIProviderConfig) {
    super(config);
    this.apiKey = config.apiKey;
    this.baseUrl = (config.baseUrl || 'https://main.codesuc.top/api').replace(/\/$/, '');
    console.log(`[Codesuc] Init: baseUrl=${this.baseUrl}, apiKey=${this.apiKey?.slice(0, 10)}...`);
  }

  async probeCapabilities(): Promise<{ tools: boolean; stream: boolean }> { // 能力探测（启动时调用一次）
    ...
    const testTool = [{ name: 'test', description: 'test', input_schema: { type: 'object', properties: {} } }];
    try { // 测试 tools 支持
      const body = { model: 'claude-opus-4-5-20251101', max_tokens: 10, messages: testMsg, tools: testTool };
      await this.request(body, false);
      this.capabilitiesCache.tools = true;
      this.supportsTools = true;
      console.log(`[Codesuc] Tools supported: YES`);
    } catch (e: any) {
      if (e.message?.includes('400') || e.message?.includes('渠道')) { this.capabilitiesCache.tools = false; this.supportsTools = false; console.log(`[Codesuc] Tools supported: NO (${e.message})`); }
      else { console.log(`[Codesuc] Tools probe error: ${e.message}`); }
    }
  ...
  private async request(body: any, stream: boolean): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.on('response', (response) => {
        ...
        if (stream) { resolve({ response, statusCode }); return; }
        response.on('data', (chunk) => { responseData += chunk.toString(); });
        response.on('end', () => {
          if (statusCode !== 200) { reject(new Error(`API Error ${statusCode}: ${responseData}`)); return; }
          try { resolve(JSON.parse(responseData)); } catch { reject(new Error(`Parse error: ${responseData}`)); }
        });
        response.on('error', reject);
      });
      request.on('error', (err) => { console.error(`[Codesuc] Request error:`, err); reject(err); });
  ...
  async chat(messages: ChatMessage[]): Promise<string> {
    ...
    const body = { model: this.getApiModel(), max_tokens: this.getMaxTokens(), system, messages: chatMsgs };
  ...
  async chatStream(messages: ChatMessage[], callbacks: StreamCallbacks): Promise<void> {
    ...
    let fullText = '';
    try {
      const { response, statusCode } = await this.request(body, true) as { response: any; statusCode: number };
      if (statusCode !== 200) {
        let errData = '';
        response.on('data', (c: Buffer) => errData += c.toString());
        response.on('end', () => { console.error(`[Codesuc] Stream Error ${statusCode}: ${errData}`); callbacks.onError(new Error(`API Error ${statusCode}: ${errData}`)); });
        return;
      }
      let buffer = '';
      let chunkCount = 0;
  ...
  async chatWithTools(messages: ChatMessage[], tools: ToolSchema[], callbacks: ToolCallbacks): Promise<void> { // 纯文本 Agent Loop（不发送 tools 字段）
    const toolsPrompt = this.buildToolsPrompt(tools); // 将 tools 转为纯文本描述嵌入 system prompt
    const systemMsg = messages.find(m => m.role === 'system');
    const enhancedSystem = [{ type: 'text', text: `${systemMsg?.content || ''}\n\n${toolsPrompt}` }];
    const chatMsgs = messages.filter(m => m.role !== 'system').map(m => {
      if (m.role === 'tool') return { role: 'user', content: `[Tool Result: ${m.toolCallId}]\n${m.content}` }; // 工具结果转为纯文本
      if (m.toolCalls?.length) return { role: 'assistant', content: m.toolCalls.map(tc => `<tool_call>\n<name>${tc.name}</name>\n<arguments>${JSON.stringify(tc.arguments, null, 2)}</arguments>\n</tool_call>`).join('\n') };
      return { role: m.role, content: m.content };
    });
    const body = { model: this.getApiModel(), max_tokens: this.getMaxTokens(), stream: true, system: enhancedSystem, messages: chatMsgs }; // 不传 tools 字段
    let fullText = '';
    try {
      const { response, statusCode } = await this.request(body, true) as { response: any; statusCode: number };
      if (statusCode !== 200) {
        let errData = '';
        response.on('data', (c: Buffer) => errData += c.toString());
        response.on('end', () => callbacks.onError(new Error(`API Error ${statusCode}: ${errData}`)));
        return;
      }
      let buffer = '';
      let chunkCount = 0;
      ...
      response.on('data', (chunk: Buffer) => {
        const chunkStr = chunk.toString();
        chunkCount++;
        if (chunkCount <= 3) console.log(`[Codesuc] Chunk #${chunkCount} (${chunkStr.length} bytes):`, chunkStr.slice(0, 200));

        // 检测 API 返回的错误 JSON（状态码200但内容是错误）
        if (chunkCount === 1 && chunkStr.includes('"error"') && chunkStr.includes('"type":"error"')) {
          try {
      ...
      response.on('end', () => {
        console.log(`[Codesuc] Stream end: ${chunkCount} chunks, fullText=${fullText.length} chars`);
      ...
      response.on('error', (e: Error) => callbacks.onError(e));
    } catch (error: any) { console.error(`[Codesuc] chatWithTools error:`, error?.message); callbacks.onError(error); }
  }

  private buildToolsPrompt(tools: ToolSchema[]): string { // 将 tools schema 转为纯文本描述
    const toolDescs = tools.map(t => `### ${t.name}\n${t.description}\n参数: ${JSON.stringify(t.parameters, null, 2)}`).join('\n\n');
    return `# 可用工具\n你可以调用以下工具完成任务。调用时使用 <tool_call> 标签：\n<tool_call>\n<name>工具名</name>\n<arguments>{"参数名": "值"}</arguments>\n</tool_call>\n\n${toolDescs}`;
  }

  private parseToolCalls(text: string): ToolCallInfo[] { // 从模型输出中解析 <tool_call> 标签
    const calls: ToolCallInfo[] = [];
    const regex = /<tool_call>\s*<name>([^<]+)<\/name>\s*<arguments>([\s\S]*?)<\/arguments>\s*<\/tool_call>/g;
    let match;
    while ((match = regex.exec(text)) !== null) {
      try { calls.push({ id: `tc_${Date.now()}_${calls.length}`, name: match[1].trim(), arguments: JSON.parse(match[2].trim()) }); } catch {}
    }
    return calls;
  }
}
