...
class RollbackManager {
  ...
  recordChange(change: Omit<ChangeRecord, "id" | "timestamp">): string | null {
    // 记录变更
  ...
  async rollback(changeId: string): Promise<{ success: boolean; error?: string }> {
    // 回滚单个变更
      ...
      if (change) {
        try {
          if (change.operation === "write")
            await (window as any).mindcode?.fs?.writeFile?.(change.path, change.originalContent);
          else if (change.operation === "delete")
            await (window as any).mindcode?.fs?.writeFile?.(change.path, change.originalContent);
          else if (change.operation === "rename" && change.oldPath)
            await (window as any).mindcode?.fs?.rename?.(change.path, change.oldPath);
  ...
  getSessionChanges(sessionId?: string): ChangeRecord[] {
    // 获取会话变更
    const sid = sessionId || this.currentSessionId;
    return sid ? this.sessions.get(sid)?.changes || [] : [];
  }

  getAllSessions(): RollbackSession[] {
