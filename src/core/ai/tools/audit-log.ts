...
class AuditLogger {
  ...
  getEntries(filter?: { action?: AuditAction; since?: number; limit?: number }): AuditEntry[] {
    // 获取日志
  ...
  getStats(): {
    ...
    return {
      ...
      byAction,
      successRate: this.entries.length ? successCount / this.entries.length : 0,
  ...
  formatForDisplay(entries?: AuditEntry[]): string {
    // 格式化显示
...
export function logToolCall(
  ...
  auditLogger.log(
    "tool_call",
    { tool: toolName, args, result: result ? JSON.stringify(result).slice(0, 200) : undefined },
    success,
    error,
  );
}
...
export function logFileOperation(
  ...
  // 快捷：记录文件操作
  const action =
    operation === "read" ? "file_read" : operation === "write" ? "file_write" : "file_delete";
