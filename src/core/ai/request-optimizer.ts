...
export async function warmupConnections(): Promise<void> { // 预热 TCP 连接 (DNS + TLS)
  if (warmedUp) return;
  warmedUp = true;
  const start = Date.now();
  await Promise.allSettled(Object.entries(warmupEndpoints).map(async ([name, url]) => {
    try { await fetch(url, { method: 'HEAD', signal: AbortSignal.timeout(2000) }); }
    catch { /* 忽略错误，只需建立连接 */ }
  }));
...
export class StreamBuffer {
  ...
  constructor(onFlush: (text: string) => void, batchMs = 16) { // 默认 ~60fps
  ...
  destroy(): void { this.flush(); }
}

// ==================== 首次响应优化 ====================
export class FirstTokenOptimizer {
  private pendingRequests: Map<string, { resolve: (v: string) => void; reject: (e: Error) => void }[]> = new Map();
...
// 初始化时预热连接
if (typeof window === 'undefined') { // 仅在 Node.js (主进程) 中运行
