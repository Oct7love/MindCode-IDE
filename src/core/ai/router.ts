...
import { ClaudeProvider, OpenAIProvider, GeminiProvider, DeepSeekProvider } from './providers';
...
export class AIRouter {
  ...
  private initializeProviders(registry: ProviderRegistry): void {
    if (registry.claude) {
      this.providers.set('claude', new ClaudeProvider(registry.claude));
    }
    if (registry.openai) {
      this.providers.set('openai', new OpenAIProvider(registry.openai));
    }
    if (registry.gemini) {
      this.providers.set('gemini', new GeminiProvider(registry.gemini));
    }
    if (registry.deepseek) {
      this.providers.set('deepseek', new DeepSeekProvider(registry.deepseek));
    }
  }
