...
import { classifyIntent, extractIntentFromComment, IntentResult, IntentType } from './intent-classifier';
 ...
 * 从 Monaco 诊断提取
 */
export function extractDiagnosticsFromMonaco(
  monaco: any,
  model: any
): DiagnosticInfo[] {
...
export async function buildCompletionContext(
  ...
  // 提取符号
  const symbols = extractSymbolsFromCode(code, language);
  ...
  if (readFile && listFiles) {
    relatedSnippets = await findRelatedSnippets(
      filePath,
      symbols,
      readFile,
      listFiles
...
export function formatDiagnosticsForPrompt(diagnostics: DiagnosticInfo[]): string {
  ...
  return diagnostics
    .slice(0, 10)
    .map(d => `[${d.severity.toUpperCase()}] Line ${d.line}: ${d.message}${d.code ? ` (${d.code})` : ''}`)
    .join('\n');
}
