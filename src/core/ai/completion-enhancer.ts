...
import type { CompletionRequest, CompletionResult, CompletionCandidate } from './completion-service';
...
export async function multiModelCompletion(
  ...
  const successResults = results
    .filter((r): r is PromiseFulfilledResult<NonNullable<Awaited<typeof modelPromises[0]>>> => 
...
function selectBestResult(
  ...
  const scored = results.map(r => {
    ...
    // 质量得分（候选数量和长度）
    const qualityScore = r.result.candidates.length > 0 
      ? Math.min(1, r.result.candidates[0].text.length / 100) 
      : 0;
    ...
    try {
      ...
      if (keywords.length > 0) {
        ...
        if (symbolResult.success && symbolResult.data) {
          context.relatedSymbols = symbolResult.data.map((s: any) => ({
            name: s.name,
...
function extractKeywords(code: string): string[] {
  ...
  const keywords = ['function', 'const', 'let', 'var', 'if', 'else', 'for', 'while', 
    'return', 'import', 'export', 'from', 'class', 'interface', 'type', 'async', 'await'];
 ...
 * 检查是否有语法错误等
 */
export function validateCompletion(completion: string, language: string): {
  valid: boolean;
  issues: string[];
} {
  const issues: string[] = [];
