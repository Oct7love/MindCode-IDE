...
export function detectTaskType(
  ...
  for (const pattern of COMPLEX_TASK_PATTERNS) {
    if (pattern.test(msg)) {
      // 匹配到复杂任务关键词
      if (/写一个|实现一个|创建一个|生成.*代码|帮我写|write.*code|implement|create.*function|添加|修改/.test(msg)) {
...
export class ModelRouter {
  ...
  route(
    ...
    const reason = model === this.primaryModel ? 'using_primary' : `routed_to_${TASK_TO_TIER[taskType]}`;
...
export function getModelRouter(primaryModel?: string): ModelRouter {
  if (!defaultRouter || (primaryModel && defaultRouter['primaryModel'] !== primaryModel)) {
    defaultRouter = new ModelRouter(primaryModel || 'claude-opus-4-5-20251101');
  }
