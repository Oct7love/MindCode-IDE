...
export const CodeEditor: React.FC<CodeEditorProps> = ({
  ...
  // 注册 Ghost Text 补全提供者（流式补全 via IPC）
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleThemeChange = (
      event: CustomEvent<{ themeId: string; editorTheme: string; type: string }>,
    ) => {
        ...
        monaco.editor.defineTheme(themeId, {
          ...
          colors: {
            ...
            "editor.selectionBackground":
              getColor("--vscode-list-activeSelectionBackground") ||
              (isDark ? "#264F78" : "#add6ff"),
            "editorLineNumber.foreground":
              getColor("--color-text-muted") || (isDark ? "#525252" : "#858585"),
            "editorLineNumber.activeForeground":
              getColor("--color-text-secondary") || (isDark ? "#A3A3A3" : "#333333"),
            "editorCursor.foreground": getColor("--color-accent-primary") || "#8b5cf6",
            "editorWidget.background":
              getColor("--color-bg-elevated") || (isDark ? "#1A1A1A" : "#f3f3f3"),
    ...
    editor.addAction({
      ...
      run: (ed) => {
        ed.trigger("mindcode", "editor.action.inlineSuggest.commit", {});
      },
    });
    ...
    editor.addAction({
      ...
      run: (ed) => {
        ed.trigger("mindcode", "editor.action.inlineSuggest.hide", {});
      },
    });
    // Alt+\ - 手动触发补全
  ...
  const handleAIChat = useCallback(
    (
      ...
      if (onAIEdit) {
        ...
        window.mindcode.ai.chatStream(
          "claude-sonnet-4-5-20250929",
          [{ role: "user", content: prompt }],
          callbacks,
        ); // 内联编辑使用 Claude 4.5 Sonnet
      } else {
