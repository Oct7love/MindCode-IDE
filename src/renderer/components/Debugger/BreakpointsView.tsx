          ...
          breakpoints.map((bp) => (
            <div key={bp.id} className={`breakpoint-item ${!bp.enabled ? "disabled" : ""}`}>
              <div className="breakpoint-main">
                ...
                />
                <span className="breakpoint-icon">{bp.verified ? "🔴" : "⚪"}</span>
                <div className="breakpoint-info">
                  ...
                  {bp.condition && (
                    <div className="breakpoint-condition">Condition: {bp.condition}</div>
                  )}
                  {bp.hitCondition && (
                    <div className="breakpoint-hit">Hit count: {bp.hitCondition}</div>
                  )}
                  {bp.logMessage && <div className="breakpoint-log">Log: {bp.logMessage}</div>}
                </div>
                <button
