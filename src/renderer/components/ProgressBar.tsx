...
export const ProgressProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const update = useCallback((id: string, progress: number) => {
    setTasks((prev) => {
      ...
      if (m.has(id)) m.set(id, Math.min(100, progress));
      return m;
    });
  }, []);
  ...
  const avgProgress = isLoading
    ? Array.from(tasks.values()).reduce((a, b) => a + b, 0) / tasks.size
    : 0;
        ...
        >
          <div
            style={{
              ...
              width: `${avgProgress || 30}%`,
              transition: "width 0.3s ease",
              animation: avgProgress === 0 ? "indeterminate 1.5s infinite" : "none",
            }}
          />
        </div>
      )}
...
// 简单的进度条组件
export const ProgressBar: React.FC<{ progress: number; height?: number; color?: string }> = ({
  progress,
  height = 4,
