...
// 图标别名
const ExplorerIcon = AppIcons.Files;
const ExtensionIcon = AppIcons.ExtensionsIcon;
...
const mockFiles: FileNode[] = [
  {
    ...
    children: [
      ...
      { name: "core", type: "folder", children: [{ name: "ai", type: "folder", children: [] }] },
    ],
  },
...
const ExtensionsPanel: React.FC<{ onOpenMarketplace: () => void }> = ({ onOpenMarketplace }) => {
  ...
  React.useEffect(() => {
    const loadExtensions = async () => {
      ...
      const featuredList = await marketplaceService.getFeatured();
      setFeatured(featuredList.slice(0, 5));
    };
    loadExtensions();
  }, []);
  ...
  React.useEffect(() => {
    if (search) {
      marketplaceService.search(search).then((results) => setSearchResults(results.slice(0, 8)));
    } else {
      setSearchResults([]);
    }
  }, [search]);

  const displayList = search ? searchResults : featured;
    ...
    <>
      ...
      <div className="panel-content extensions-panel">
        ...
        <div className="ext-section">
          ...
          {displayList.map((ext: ExtensionInfo) => (
            <div key={ext.id} className="ext-item">
              <span className="ext-item-icon">{ext.icon || "📦"}</span>
              <div className="ext-item-info">
                <div className="ext-item-name">{ext.displayName}</div>
                <div className="ext-item-meta">
                  ⬇️{(ext.downloads / 1000).toFixed(0)}k ⭐{ext.rating}
                </div>
              </div>
              {ext.installed ? (
        ...
        {/* 打开完整市场 */}
        <button className="ext-open-marketplace" onClick={onOpenMarketplace}>
          🏪 打开扩展市场
        </button>
      </div>
    </>
...
const Sidebar: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"explorer" | "search" | "git" | "extensions">(
    "explorer",
  );
    ...
    <div className="sidebar">
      ...
      <div className="sidebar-panel">
        {activeTab === "extensions" ? (
          ...
          <>
            <div className="panel-header">
              <span className="panel-title">{panelTitles[activeTab]}</span>
            </div>
              ...
              {activeTab === "search" && (
                <div className="search-panel">
                  ...
                  <div className="search-options">
                    <label>
                      <input type="checkbox" /> 区分大小写
                    </label>
                    <label>
                      <input type="checkbox" /> 全词匹配
                    </label>
                    <label>
              ...
              {activeTab === "git" && (
                <div className="git-panel">
                  <div className="git-message">
                    <textarea placeholder="提交消息..." rows={3}></textarea>
                  </div>
                  <button className="git-commit-btn">✓ 提交</button>
                  <div className="git-section">
                    <div className="git-section-title">更改</div>
                    <div className="git-empty">暂无更改</div>
                  </div>
                </div>
              )}
            </div>
