...
import type { CodeIssue, ReviewResult } from "../../../core/review/codeReviewService";
import { codeReviewService } from "../../../core/review/codeReviewService";
...
export const ReviewPanel: React.FC<ReviewPanelProps> = ({
  ...
  // 自动审查
  useEffect(() => {
    if (isOpen && files) runReview();
  }, [isOpen, files, runReview]);
  ...
  const handleIssueClick = (issue: CodeIssue) => {
    onNavigateToIssue?.(issue.file, issue.line);
  };
    ...
    <div className="review-panel-overlay" onClick={onClose}>
      <div className="review-panel" onClick={(e) => e.stopPropagation()}>
        ...
        {result && (
          <div className="review-summary">
            <div className="summary-item error">
              <span className="count">{result.summary.errors}</span>
              <span className="label">错误</span>
            </div>
            <div className="summary-item warning">
              <span className="count">{result.summary.warnings}</span>
              <span className="label">警告</span>
            </div>
            <div className="summary-item info">
              <span className="count">{result.summary.infos}</span>
              <span className="label">提示</span>
            </div>
            <div className="summary-item total">
              <span className="count">{result.reviewedFiles.length}</span>
        ...
        <div className="review-filters">
          ...
          <button className="refresh-btn" onClick={runReview} disabled={isReviewing}>
        ...
        <div className="review-issues">
          ...
          {!isReviewing && filteredIssues.length === 0 && (
            <div className="review-empty">✅ 没有发现问题</div>
          )}
