...
export const BookmarkManager: React.FC<BookmarkManagerProps> = ({
  ...
  const isCurrentLineBookmarked = useMemo(
    () =>
      currentFile && currentLine
        ? bookmarks.some((b) => b.filePath === currentFile && b.line === currentLine)
        : false,
  ...
  const filtered = useMemo(() => {
    ...
    return bookmarks.filter(
      (b) => b.filePath.toLowerCase().includes(lower) || b.label?.toLowerCase().includes(lower),
    );
  }, [bookmarks, filter]);
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      >
        <button
          ...
          title={isCurrentLineBookmarked ? "移除书签" : "添加书签"}
          style={{
            flex: 1,
            padding: "6px",
            background: isCurrentLineBookmarked ? "#f59e0b" : "var(--color-bg-hover)",
                ...
                .map((bm) => (
                  <div
                    ...
                    style={{
                      ...
                      borderBottom: "1px solid var(--color-border)",
                      background:
                        bm.filePath === currentFile && bm.line === currentLine
                          ? "var(--color-bg-hover)"
                          : "transparent",
                    }}
                  >
...
export function useBookmarks() {
  ...
  const getBookmarksForFile = useCallback(
    (filePath: string) => bookmarks.filter((b) => b.filePath === filePath),
    [bookmarks],
  );
  const hasBookmark = useCallback(
    (filePath: string, line: number) =>
      bookmarks.some((b) => b.filePath === filePath && b.line === line),
    [bookmarks],
  );
