...
import type { Keybinding, KeybindingConflict } from "../../services/keybindingService";
...
export const KeybindingsPanel: React.FC = () => {
  ...
  // Vim 模式切换
  const toggleVim = () => {
    keybindingService.setVimMode(!vimEnabled);
  };
          ...
          {conflicts.map((c) => (
            <div key={c.key} className="conflict-item">
              ...
              <span className="conflict-commands">
                {c.bindings.map((b) => b.command).join(", ")}
              </span>
            </div>
          ))}
        </div>
      ...
      <div className="keybindings-list">
        ...
        {filtered.map((b) => (
          <div key={b.id} className={`keybinding-row${editingId === b.id ? " editing" : ""}`}>
            <span className="col-command" title={b.command}>
              {b.command}
            </span>
            ...
            <span className="col-source">
              {b.source === "user" ? "自定义" : b.source === "extension" ? "扩展" : "默认"}
            </span>
            <span className="col-actions">
              {b.source === "user" && (
                <button
                  className="btn-reset"
                  onClick={() => keybindingService.reset(b.id)}
                  title="重置为默认"
                >
                  ↺
                </button>
