...
export const TerminalPanel: React.FC<TerminalPanelProps> = ({ onCommand, onClose, onNew }) => {
  const [terminals, setTerminals] = useState<TerminalInstance[]>([
    {
      ...
      name: "Terminal 1",
      cwd: "~",
      output: ["Welcome to MindCode Terminal\n"],
      isRunning: false,
    },
  ]);
  ...
  const addTerminal = () => {
    ...
    setTerminals((prev) => [
      ...prev,
      { id, name: `Terminal ${prev.length + 1}`, cwd: "~", output: [], isRunning: false },
    ]);
    setActiveId(id);
    onNew?.();
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTerminals((prev) =>
      prev.map((t) =>
        t.id === activeId ? { ...t, output: [...t.output, `$ ${cmd}\n`], isRunning: true } : t,
      ),
    );
    ...
    try {
        ...
        setTerminals((prev) =>
          prev.map((t) =>
            t.id === activeId
              ? {
                  ...t,
                  output: [...t.output, result.output || "", result.error || ""],
                  isRunning: false,
      ...
      setTerminals((prev) =>
        prev.map((t) =>
          t.id === activeId
            ? { ...t, output: [...t.output, `Error: ${(e as Error).message}\n`], isRunning: false }
            : t,
        ),
      );
    }
  ...
  const clear = () => {
    setTerminals((prev) => prev.map((t) => (t.id === activeId ? { ...t, output: [] } : t)));
  };
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--color-bg-primary)]">
        ...
        <div className="flex-1 flex items-center overflow-x-auto">
          {terminals.map((t) => (
            <div
              ...
              className={`flex items-center gap-2 px-3 py-1.5 cursor-pointer border-r border-[var(--color-border)] ${activeId === t.id ? "bg-[var(--color-bg-primary)]" : "hover:bg-[var(--color-bg-hover)]"}`}
            >
      ...
      <div
        ref={outputRef}
        className="flex-1 overflow-auto p-2 font-mono text-sm bg-[#1e1e1e] text-[#d4d4d4]"
      >
      ...
      <form
        ...
        className="flex items-center border-t border-[var(--color-border)] bg-[#1e1e1e]"
      >
        ...
        <input
          ...
          className="flex-1 py-2 bg-transparent text-[#d4d4d4] font-mono text-sm outline-none"
          placeholder="输入命令..."
          autoFocus
...
export const OutputPanel: React.FC<{ entries: OutputEntry[]; onClear?: () => void }> = ({
  entries,
  onClear,
}) => {
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 p-2 border-b border-[var(--color-border)]">
        ...
        {onClear && (
          <button
            onClick={onClear}
            className="px-2 py-1 text-sm bg-[var(--color-bg-hover)] rounded"
          >
            清空
          </button>
        )}
      ...
      <div ref={ref} className="flex-1 overflow-auto font-mono text-xs">
        {filtered.map((e, i) => (
          <div
            key={i}
            className={`flex items-start gap-2 px-2 py-1 hover:bg-[var(--color-bg-hover)] ${levelColors[e.level]}`}
          >
            <span className="text-[var(--color-text-muted)] w-16 flex-shrink-0">
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 p-2 border-b border-[var(--color-border)]">
        <button
          ...
          className={`px-2 py-1 text-sm rounded ${filter === "all" ? "bg-[var(--color-accent-primary)] text-white" : "bg-[var(--color-bg-hover)]"}`}
        >
          全部 ({problems.length})
        </button>
        <button
          onClick={() => setFilter("error")}
          className={`px-2 py-1 text-sm rounded flex items-center gap-1 ${filter === "error" ? "bg-[var(--color-error)] text-white" : "bg-[var(--color-bg-hover)]"}`}
        >
          ❌ {errorCount}
        </button>
        <button
          onClick={() => setFilter("warning")}
          className={`px-2 py-1 text-sm rounded flex items-center gap-1 ${filter === "warning" ? "bg-[var(--color-warning)] text-white" : "bg-[var(--color-bg-hover)]"}`}
        >
          ⚠️ {warningCount}
        </button>
      </div>
      <div className="flex-1 overflow-auto">
        {Object.entries(grouped).map(([file, fileProblems]) => (
          <div key={file}>
            <div className="px-2 py-1 text-sm font-medium bg-[var(--color-bg-secondary)] sticky top-0">
              {file}
            </div>
            {fileProblems.map((p, i) => (
              <div
                ...
                className={`flex items-start gap-2 px-3 py-1.5 cursor-pointer hover:bg-[var(--color-bg-hover)] ${severityColor[p.severity]}`}
              >
