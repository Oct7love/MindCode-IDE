...
- 考虑技术风险和依赖关系`;
  ...
  const toggleMilestone = useCallback(
    (mId: string) => {
      if (currentPlan)
        setPlan({
          ...currentPlan,
          milestones: currentPlan.milestones.map((m) =>
  ...
  const executeInAgent = useCallback(() => {
    if (currentPlan) {
      setPlan({ ...currentPlan, status: "executing" });
      setMode("agent");
    }
  }, [currentPlan, setPlan, setMode]);
    ...
    <div className="ai-plan-view">
        ...
        {messages.length === 0 && !streamingText && (
          <div className="ai-empty-state">
            ...
            <div className="ai-empty-desc">
              描述你的需求，我会帮你制定详细的开发计划
              <br />
              计划确认后可以一键切换到 Agent 执行
            </div>
          </div>
        )}
        {messages.map((msg) => (
          <div key={msg.id} className={`ai-plan-msg ai-plan-msg-${msg.role}`}>
            ...
            <div className="ai-plan-msg-body">
              <div className="ai-plan-msg-content">
      ...
      {currentPlan && (
        <div className="ai-plan-card">
          <div className="ai-plan-card-header">
            ...
            <div className="ai-plan-card-status">
          ...
          <div className="ai-plan-card-progress">
            <div className="ai-plan-card-progress-bar">
              <div className="ai-plan-card-progress-fill" style={{ width: `${progress}%` }} />
            </div>
            <span className="ai-plan-card-progress-text">
          ...
          <div className="ai-plan-card-tasks">
            {currentPlan.tasks.slice(0, 5).map((t) => (
              <div
                key={t.id}
                className={`ai-plan-card-task ${t.completed ? "completed" : ""}`}
            ...
            {currentPlan.tasks.length > 5 && (
              <div className="ai-plan-card-more">+{currentPlan.tasks.length - 5} 更多任务</div>
            )}
          </div>
          <div className="ai-plan-card-actions">
            <button
              className="ai-plan-card-btn"
              onClick={() =>
                setPlan({
                  ...currentPlan,
                  status: currentPlan.status === "locked" ? "draft" : "locked",
                })
              }
            >
              {currentPlan.status === "locked" ? "解锁" : "锁定"}
            </button>
            <button
              className="ai-plan-card-btn primary"
              onClick={executeInAgent}
      ...
      <div className="ai-plan-composer">
        <textarea
          ...
          placeholder="描述你要开发的功能... (Enter 发送)"
        ...
        <div className="ai-plan-composer-footer">
          <div className="ai-plan-composer-left">
            ...
            {messages.length > 0 && (
              <button className="ai-plan-clear-btn" onClick={clearChat} disabled={isGenerating}>
                清空
              </button>
            )}
          </div>
