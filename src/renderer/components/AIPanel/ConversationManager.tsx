...
export const ConversationManager: React.FC<ConversationManagerProps> = ({
  ...
  const handleSaveCurrent = useCallback(() => {
    ...
    if (exists) {
      saveConversations(
        conversations.map((c) =>
          c.id === currentConversation.id ? { ...currentConversation, updatedAt: Date.now() } : c,
        ),
      );
    } else {
  ...
  const exportAll = useCallback(() => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `mindcode-conversations-${Date.now()}.json`;
    a.click();
    URL.revokeObjectURL(url);
  }, [conversations]);
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
          {/* 会话列表 */}
          <div
            style={{ width: 280, borderRight: "1px solid var(--color-border)", overflow: "auto" }}
          >
            {filtered.length === 0 ? (
              <div
                style={{
              ...
              filtered.map((conv) => (
                <div
                  ...
                  style={{
                    padding: "12px",
                    cursor: "pointer",
                    background: selected === conv.id ? "var(--color-bg-hover)" : "transparent",
                    borderBottom: "1px solid var(--color-border)",
                  }}
                >
                  <div
              ...
              <>
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    ...
                    <button
                      onClick={() => exportConversation(preview)}
                      style={{
                        padding: "6px 12px",
                        background: "transparent",
                        border: "1px solid var(--color-border)",
                    ...
                    <button
                      onClick={() => deleteConversation(preview.id)}
                      style={{
                        padding: "6px 12px",
                        background: "#ef4444",
                        color: "#fff",
                  ...
                  {preview.messages.map((msg, idx) => (
                    <div
                      ...
                      style={{
                        ...
                        padding: 12,
                        background:
                          msg.role === "user" ? "var(--color-bg-base)" : "var(--color-bg-hover)",
                        borderRadius: 8,
                      }}
                    >
                      <div
                      ...
                      <div
                        style={{ fontSize: 13, whiteSpace: "pre-wrap", wordBreak: "break-word" }}
                      >
