...
import { copyToClipboard, CopyResult } from './utils/copyService';
...
export const CopyButton: React.FC<CopyButtonProps> = memo(({
  ...
  // 根据状态获取文案
  const label = state === 'success' ? successLabel 
              : state === 'error' ? 'Failed' 
              : defaultLabel;

  // 动态 title
  const buttonTitle = state === 'success' ? 'Copied to clipboard!' 
  ...
  return (
    ...
    >
      ...
      {showLabel && (
        <span className="copy-btn-label">{label}</span>
      )}
    </button>
  );
});
