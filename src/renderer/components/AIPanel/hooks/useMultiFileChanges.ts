...
import { FileChange } from '../MultiFileChanges';
...
function calculateStats(original: string, modified: string): { additions: number; deletions: number } {
...
export function useMultiFileChanges(): UseMultiFileChangesReturn {
  ...
  // 添加单个变更
  const addChange = useCallback((change: Omit<FileChange, 'id' | 'status' | 'additions' | 'deletions'>) => {
  ...
  // 添加多个变更
  const addChanges = useCallback((newChanges: Omit<FileChange, 'id' | 'status' | 'additions' | 'deletions'>[]) => {
  ...
  const acceptChange = useCallback(async (id: string): Promise<boolean> => {
    ...
    try {
      const fullPath = change.filePath.startsWith('/') || change.filePath.match(/^[a-zA-Z]:/)
        ? change.filePath
        : workspaceRoot 
