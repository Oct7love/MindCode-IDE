...
export const highlightCode = (code: string, language: string): React.ReactNode[] => {
  ...
  return lines.map((line, lineIndex) => {
      ...
      const cls = line.startsWith("+")
        ...
        : line.startsWith("-")
          ...
          : line.startsWith("@@")
            ...
            : "";
      return (
        <div key={lineIndex} className={`code-line ${cls}`}>
    ...
    while (remaining.length > 0) {
      ...
      if (commentMatch) {
        tokens.push(
          <span key={`${lineIndex}-${tokenIndex++}`} className="syntax-comment">
            {commentMatch[0]}
          </span>,
        );
        remaining = "";
      ...
      if (doubleStringMatch) {
        tokens.push(
          <span key={`${lineIndex}-${tokenIndex++}`} className="syntax-string">
            {doubleStringMatch[0]}
          </span>,
        );
      ...
      if (numberMatch) {
        tokens.push(
          <span key={`${lineIndex}-${tokenIndex++}`} className="syntax-number">
            {numberMatch[0]}
          </span>,
        );
      ...
      for (const keyword of langKeywords) {
        ...
        if (keywordMatch) {
          tokens.push(
            <span key={`${lineIndex}-${tokenIndex++}`} className="syntax-keyword">
              {keywordMatch[0]}
            </span>,
          );
      ...
      if (funcMatch) {
        tokens.push(
          <span key={`${lineIndex}-${tokenIndex++}`} className="syntax-function">
            {funcMatch[1]}
          </span>,
        );
      ...
      if (identMatch) {
        tokens.push(
          <span key={`${lineIndex}-${tokenIndex++}`} className="syntax-variable">
            {identMatch[0]}
          </span>,
        );
      ...
      if (operatorMatch) {
        tokens.push(
          <span key={`${lineIndex}-${tokenIndex++}`} className="syntax-operator">
            {operatorMatch[0]}
          </span>,
        );
...
export const CodeBlock: React.FC<CodeBlockProps> = memo(
  ({ language, code, filename, maxLines = 15, onCopy, onApply, onPreview, onOpenInEditor }) => {
    ...
    const displayCode =
      shouldCollapse && !expanded ? lines.slice(0, maxLines - 2).join("\n") : code;
    ...
    return (
      <div
        className={`code-block ${isDiff ? "code-block--diff" : ""} ${applied ? "code-block--applied" : ""}`}
      >
        ...
        >
          <pre>
            <code>{highlightCode(displayCode, language)}</code>
          </pre>
        </div>
        {shouldCollapse && (
          <button className="code-block-toggle" onClick={() => setExpanded(!expanded)}>
            {expanded ? (
              ...
              <>
                <ChevronDownIcon /> 展开剩余 {hiddenLines} 行
              </>
            )}
          </button>
        )}
...
const Callout: React.FC<{ type: CalloutType; children: React.ReactNode }> = ({
  type,
  children,
}) => (
...
const parseMarkdown = (text: string): ParsedBlock[] => {
  ...
  while (i < lines.length) {
    const line = lines[i];
    // 代码块（支持 ```lang:filepath 格式）
    if (line.startsWith("```")) {
      const langRaw = line.slice(3).trim();
      let lang = langRaw || "text";
      let filename: string | undefined;
      // 解析 lang:filepath 格式
      const colonIdx = langRaw.indexOf(":");
      if (colonIdx > 0 && !langRaw.startsWith("//")) {
        lang = langRaw.slice(0, colonIdx);
        filename = langRaw.slice(colonIdx + 1).trim();
      }
...
const renderInline = (text: string): React.ReactNode => {
  ...
  while (remaining.length > 0) {
    ...
    if (strikeMatch) {
      parts.push(<del key={idx++}>{strikeMatch[1]}</del>);
...
const renderTable = (content: string): React.ReactNode => {
  // 表格渲染
...
export const MarkdownRenderer: React.FC<MarkdownProps> = memo(
  ({ content, onApplyCode, onPreviewCode, onOpenInEditor }) => {
      ...
      <div className="markdown-content">
        {blocks.map((block, i) => {
          switch (block.type) {
            case "code":
              return (
                <CodeBlock
                  ...
                  onApply={
                    onApplyCode
                      ? () => onApplyCode(block.content, block.language || "text")
                      : undefined
                  }
                  onPreview={
                    onPreviewCode
                      ? () => onPreviewCode(block.content, block.language || "text")
                      : undefined
                  }
            ...
            case "callout":
              return (
                <Callout key={i} type={block.calloutType!}>
...
const SpinnerIcon = () => (
  <svg viewBox="0 0 16 16" width="14" height="14" fill="currentColor" className="spin">
    <path d="M8 0a8 8 0 100 16A8 8 0 008 0zm.75 2.015v2.993a.75.75 0 01-1.5 0V2.015A6.502 6.502 0 0114 8a.75.75 0 01-1.5 0A5 5 0 007.25 2.015z" />
  </svg>
);
const EditorIcon = () => (
  <svg viewBox="0 0 16 16" width="14" height="14" fill="currentColor">
