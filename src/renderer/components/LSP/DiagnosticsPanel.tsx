...
export const DiagnosticsPanel: React.FC<DiagnosticsPanelProps> = ({ onJumpToLocation }) => {
  ...
  const getSeverityColor = (severity: number) => {
    switch (severity) {
      case 1: return 'var(--vscode-errorForeground)';
      case 2: return 'var(--vscode-warningForeground)';
      case 3: return 'var(--vscode-infoForeground)';
      case 4: return 'var(--vscode-descriptionForeground)';
  ...
  const filteredDiagnostics = diagnostics.map(group => ({
    ...group,
  ...
  const getErrorCount = () => {
    return diagnostics.reduce((sum, group) => 
      sum + group.diagnostics.filter(d => d.severity === 1).length, 0
    );
  };

  const getWarningCount = () => {
    return diagnostics.reduce((sum, group) => 
      sum + group.diagnostics.filter(d => d.severity === 2).length, 0
    );
  };
                      ...
                      >
                        ...
                        <div className="diagnostic-content">
                          ...
                          <div className="diagnostic-location">
                            ...
                            {diagnostic.source && <span className="diagnostic-source"> • {diagnostic.source}</span>}
