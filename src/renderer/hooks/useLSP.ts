...
import type { LSPClient } from "../../core/lsp";
import { createLSPClient } from "../../core/lsp";
...
export function useLSP(
  ...
  const getClient = useCallback(
    async (language: string): Promise<LSPClient | null> => {
  ...
  const syncDocument = useCallback(
    async (model: monaco.editor.ITextModel) => {
  ...
  const goToDefinition = useCallback(
    async (
      ...
      const uri = `file://${model.uri.path.replace(/\\/g, "/")}`;
      return client.getDefinition(uri, {
  ...
  const getHover = useCallback(
    async (model: monaco.editor.ITextModel, position: monaco.Position): Promise<Hover | null> => {
      const client = await getClient(model.getLanguageId());
      if (!client) return null;
      const uri = `file://${model.uri.path.replace(/\\/g, "/")}`;
      return client.getHover(uri, {
        line: position.lineNumber - 1,
        character: position.column - 1,
      });
    },
  ...
  const getCompletion = useCallback(
    async (
      ...
      if (!client) return [];
      const uri = `file://${model.uri.path.replace(/\\/g, "/")}`;
      return client.getCompletion(uri, {
        line: position.lineNumber - 1,
        character: position.column - 1,
      });
    },
  ...
  const findReferences = useCallback(
    async (model: monaco.editor.ITextModel, position: monaco.Position): Promise<Location[]> => {
      const client = await getClient(model.getLanguageId());
      if (!client) return [];
      const uri = `file://${model.uri.path.replace(/\\/g, "/")}`;
      return client.getReferences(uri, {
        line: position.lineNumber - 1,
        character: position.column - 1,
      });
    },
