...
export function useProjectRules(): UseProjectRulesReturn {
  ...
  // 创建新规则
  const addRule = useCallback(async (name: string, content: string): Promise<ProjectRule | null> => {
  ...
  // 更新规则内容
  const updateRuleContent = useCallback(async (id: string, content: string): Promise<boolean> => {
    const rule = rules.find(r => r.id === id);
    if (!rule) return false;
