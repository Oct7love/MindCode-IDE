...
export function useSessionStorage<T>(key: string, initialValue: T): [T, (value: T | ((prev: T) => T)) => void] {
  const [storedValue, setStoredValue] = useState<T>(() => {
    try { const item = sessionStorage.getItem(key); return item ? JSON.parse(item) : initialValue; } catch { return initialValue; }
  });
  const setValue = useCallback((value: T | ((prev: T) => T)) => {
    setStoredValue(prev => {
      const newValue = typeof value === 'function' ? (value as (prev: T) => T)(prev) : value;
...
export function useThrottle<T>(value: T, interval: number): T {
  ...
  useEffect(() => {
    const now = Date.now();
    if (now - lastUpdated.current >= interval) { lastUpdated.current = now; setThrottledValue(value); }
    else { const timer = setTimeout(() => { lastUpdated.current = Date.now(); setThrottledValue(value); }, interval - (now - lastUpdated.current)); return () => clearTimeout(timer); }
  }, [value, interval]);
  return throttledValue;
}

// useEventListener - 事件监听
export function useEventListener<K extends keyof WindowEventMap>(eventName: K, handler: (event: WindowEventMap[K]) => void, element: Window | HTMLElement | null = window): void {
...
export function useKeyPress(targetKey: string, handler: () => void): void {
  useEventListener('keydown', (e: KeyboardEvent) => { if (e.key === targetKey) handler(); });
}

// useClipboard - 剪贴板
export function useClipboard(): { copy: (text: string) => Promise<boolean>; paste: () => Promise<string>; copied: boolean } {
  const [copied, setCopied] = useState(false);
  const copy = useCallback(async (text: string) => {
    try { await navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 2000); return true; } catch { return false; }
  }, []);
  const paste = useCallback(async () => { try { return await navigator.clipboard.readText(); } catch { return ''; } }, []);
...
export function useHover<T extends HTMLElement>(): [React.RefObject<T>, boolean] {
  ...
  useEffect(() => {
    ...
    return () => { node.removeEventListener('mouseenter', handleMouseEnter); node.removeEventListener('mouseleave', handleMouseLeave); };
  }, []);
  return [ref, hovered];
}
...
export function useFocus<T extends HTMLElement>(): [React.RefObject<T>, boolean, () => void] {
  ...
  useEffect(() => {
    ...
    return () => { node.removeEventListener('focus', handleFocus); node.removeEventListener('blur', handleBlur); };
  }, []);
...
export default {
  ...
  useEventListener, useKeyPress, useClipboard, useMediaQuery, useWindowSize, usePrevious,
  useToggle, useInterval, useTimeout, useMounted, useUpdateEffect, useClickOutside, useHover, useFocus,
};
