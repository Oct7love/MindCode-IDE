...
class ShortcutManager {
  ...
  // 保存 bound 引用，以便 destroy 时 removeEventListener
  ...
  constructor() {
    ...
    this._boundBlur = () => this.pressedKeys.clear();
  ...
  private handleKeyDown(e: KeyboardEvent): void {
    ...
    for (const shortcut of this.shortcuts.values()) {
      ...
      if (this.matchCombo(combo, targetKeys)) {
        ...
        try {
          shortcut.handler();
          this.listeners.forEach((fn) => fn(shortcut));
        } catch (err) {
          console.error("[ShortcutManager] Handler error:", err);
        }
        return;
      }
    }
  ...
  private handleKeyUp(e: KeyboardEvent): void {
    this.pressedKeys.delete(this.normalizeKey(e));
  }
  ...
  onTrigger(fn: (shortcut: Shortcut) => void): () => void {
  ...
  private saveCustomBindings(): void {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(Array.from(this.customBindings.entries())));
    } catch {}
  }
