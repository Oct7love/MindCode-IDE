...
interface PerformanceReport {
  ...
  summary: { avg: number; min: number; max: number; p95: number; count: number };
}
...
class PerformanceMonitor {
  ...
  async time<T>(name: string, fn: () => T | Promise<T>, tags?: Record<string, string>): Promise<T> {
    ...
    try {
      ...
      this.record(name, performance.now() - start, tags);
    }
  }
  ...
  getReport(name: string, since?: number): PerformanceReport {
    ...
    if (values.length === 0)
      return { metrics: [], summary: { avg: 0, min: 0, max: 0, p95: 0, count: 0 } };
  ...
  subscribe(fn: (metric: PerformanceMetric) => void): () => void {
    // 订阅指标
  ...
  getWebVitals(): Record<string, number> {
    ...
    return {
      ...
      memoryUsed:
        (performance as unknown as Record<string, { usedJSHeapSize: number }>).memory
          ?.usedJSHeapSize / 1048576 || 0,
    };
  }
}
...
export function usePerformance(name: string, interval = 5000): PerformanceReport {
  const [report, setReport] = useState<PerformanceReport>({
    metrics: [],
    summary: { avg: 0, min: 0, max: 0, p95: 0, count: 0 },
  });
  useEffect(() => {
