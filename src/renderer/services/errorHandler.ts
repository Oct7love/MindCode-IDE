...
class ErrorHandler {
  ...
  init(): void {
    ...
    window.onerror = (msg, url, line, col, err) => {
      this.capture(err || new Error(String(msg)), { url, line, col });
      return true;
    };
    window.onunhandledrejection = (e) => {
      this.capture(e.reason instanceof Error ? e.reason : new Error(String(e.reason)), {
        type: "unhandledrejection",
      });
    };
  }

  capture(error: Error | string, context?: Record<string, unknown>): AppError {
    // 捕获错误
  ...
  wrap<T extends (...args: any[]) => any>(fn: T, context?: Record<string, unknown>): T {
    // 包装函数
    return ((...args: Parameters<T>) => {
      try {
        return fn(...args);
      } catch (e) {
        this.capture(e as Error, context);
        throw e;
      }
    }) as T;
  }

  async wrapAsync<T>(fn: () => Promise<T>, context?: Record<string, unknown>): Promise<T> {
    // 包装异步
    try {
      return await fn();
    } catch (e) {
  ...
  boundary<T>(fn: () => T, fallback: T, context?: Record<string, unknown>): T {
    // 错误边界
    try {
      return fn();
    } catch (e) {
  ...
  subscribe(fn: ErrorCallback): () => void {
    // 订阅错误
  ...
  getErrors(since?: number): AppError[] {
    return since ? this.errors.filter((e) => e.timestamp >= since) : this.errors;
  }
...
export class NetworkError extends AppException {
