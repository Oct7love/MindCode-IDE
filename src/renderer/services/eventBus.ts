...
class EventBus<E extends EventMap = EventMap> {
  ...
  on<K extends keyof E>(event: K, callback: EventCallback<E[K]>): () => void { // 订阅
    if (!this.listeners.has(event)) this.listeners.set(event, new Set());
    this.listeners.get(event)!.add(callback);
    return () => this.off(event, callback);
  }

  once<K extends keyof E>(event: K, callback: EventCallback<E[K]>): () => void { // 单次订阅
  ...
  off<K extends keyof E>(event: K, callback?: EventCallback<E[K]>): void { // 取消订阅
    if (!callback) { this.listeners.delete(event); this.onceListeners.delete(event); return; }
    this.listeners.get(event)?.delete(callback);
    this.onceListeners.get(event)?.delete(callback);
  }

  emit<K extends keyof E>(event: K, data: E[K]): void { // 发布
    this.history.set(event, data);
    this.listeners.get(event)?.forEach(fn => { try { fn(data); } catch (e) { console.error('[EventBus] Error:', e); } });
    const once = this.onceListeners.get(event);
    if (once) { once.forEach(fn => { try { fn(data); } catch (e) { console.error('[EventBus] Error:', e); } }); this.onceListeners.delete(event); }
  }

  async emitAsync<K extends keyof E>(event: K, data: E[K]): Promise<void> { // 异步发布
    this.history.set(event, data);
    const callbacks = [...(this.listeners.get(event) || []), ...(this.onceListeners.get(event) || [])];
    await Promise.all(callbacks.map(fn => Promise.resolve().then(() => fn(data))));
    this.onceListeners.delete(event);
  }

  getLastEvent<K extends keyof E>(event: K): E[K] | undefined { return this.history.get(event) as E[K]; } // 获取最近事件
  hasListeners<K extends keyof E>(event: K): boolean { return (this.listeners.get(event)?.size ?? 0) > 0; }
  listenerCount<K extends keyof E>(event: K): number { return this.listeners.get(event)?.size ?? 0; }
  clear(): void { this.listeners.clear(); this.onceListeners.clear(); this.history.clear(); }
  events(): (keyof E)[] { return Array.from(this.listeners.keys()); }
}
