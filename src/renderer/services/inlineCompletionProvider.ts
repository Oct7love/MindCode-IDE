...
import type { CompletionRequest } from "./completionService";
...
/** 获取 LSP 补全并提取最佳项 */
async function getLSPCompletion(
  lang: string,
  uri: string,
  line: number,
  col: number,
  ...
  return {
    provideInlineCompletions: async (
      ...
      const [lspResult, aiResult] = await Promise.all([
        getLSPCompletion(
          lang === "typescriptreact" ? "typescript" : lang,
          uri,
      ...
      let source = "ai";
      if (!completion && lspResult) {
        completion = lspResult;
        source = "lsp";
      }
      ...
      const inlineCompletion: monaco.languages.InlineCompletion = {
        ...
        command: {
          ...
          arguments: [source === "ai" ? aiResult?.model || "ai" : "lsp", aiResult?.latency_ms || 0],
        },
      };
...
export function registerInlineCompletionProvider(
  ...
  const cmdDisposable = monaco.editor.registerCommand(
    "mindcode.completionAccepted",
    (_accessor, model?: string, latencyMs?: number) => {
