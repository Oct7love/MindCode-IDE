...
class ApiClient {
  ...
  addResponseInterceptor(fn: (res: Response) => Response): void {
  ...
  async request<T>(url: string, config: RequestConfig = {}): Promise<ApiResponse<T>> {
    ...
    let cfg: RequestConfig & {
      ...
      retry: number;
      retryDelay: number;
    } = { timeout: 30000, retry: 0, retryDelay: 1000, ...config, method: config.method || "GET" };
    for (const fn of this.interceptors.request)
      cfg = { ...cfg, ...fn(cfg), method: fn(cfg).method || cfg.method };
    ...
    const fetchWithRetry = async (attempt: number): Promise<ApiResponse<T>> => {
      ...
      try {
        ...
        performanceMonitor.measure(`api:${cacheKey}`, undefined, {
          url: fullUrl,
          method: cfg.method,
        });
        if (cfg.cache && cfg.method === "GET")
        ...
        if (attempt < cfg.retry!) {
          await new Promise((r) => setTimeout(r, cfg.retryDelay! * (attempt + 1)));
