...
describe("CacheManager", () => {
  ...
  describe("TTL expiration", () => {
    it("expires after TTL", async () => {
      const shortCache = new CacheManager<string>(10, 50);
  ...
  describe("LRU eviction", () => {
    it("evicts item with lowest score when cache is full", () => {
      cache.set("a", "1");
      cache.set("b", "2");
      cache.set("c", "3");
      // 访问 'c' 和 'b' 增加它们的 hits，让 'a' 分数最低
      cache.get("c");
      cache.get("b");
      cache.set("d", "4"); // 应该淘汰 hits 最少的
      // 至少 d 和被访问过的 item 应该存在
